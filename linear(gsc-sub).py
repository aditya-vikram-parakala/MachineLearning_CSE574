...
def GenerateBigSigma(Data, MuMatrix,TrainingPercent,IsSynthetic): # generating covarience matrix
    ...
    for j in range(len(Data)):
        BigSigma[j][j] = varVect[j]+0.02 # fill the diagonal elements with the varience values and rest all are 0's and added a bias term of 0.2 to remove the singular matrix error
    if IsSynthetic == True:
...
RawTarget = GetTargetVector(r'C:\Users\aditya vikram\GSC_t_gsc_sub.csv')
RawData   = GenerateRawData(r'C:\Users\aditya vikram\GSC_X_gsc_sub.csv',IsSynthetic)
