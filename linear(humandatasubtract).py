...
M = 10  # It is the complexity of the model,we can change this acccordingly by checking the accuracy of the model to improve it later on.
PHI = []
IsSynthetic = False
...
def GenerateBigSigma(Data, MuMatrix,TrainingPercent,IsSynthetic): # generating covarience matrix
    ...
    for j in range(len(Data)):
        BigSigma[j][j] = varVect[j] # fill the diagonal elements with the varience values and rest all are 0's so its a diagonal matrix
    if IsSynthetic == True:
...
RawTarget = GetTargetVector(r'C:\Users\aditya vikram\humandata_t_hd_sub.csv')
RawData   = GenerateRawData(r'C:\Users\aditya vikram\humandata_X_hd_sub.csv',IsSynthetic)
...
# calculating the Erms on training testing and validation sets
...
print ('UBITname      = aparakal')
print ('Person Number = 50289171')
